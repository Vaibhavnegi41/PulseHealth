...
def create_access_token(data: dict):
    to_encode = data.copy()
    expires = datetime.utcnow() + timedelta(minutes=token_expires_minute)
...
def user_register(details: LoginDetails, db=Depends(get_connection)):
    try:
        ...
        db.commit()
        return {"message": "Successfully registered!", "type": "success", "Username": details.email}
    except Exception as e:
...
def login_user(register_details: LoginDetails, db=Depends(get_connection), background_tasks: BackgroundTasks):
    try:
        ...
        if not result:
            raise HTTPException(status_code=400, detail="Login credentials do not exist!")
        ...
        background_tasks.add_task(send_login_email, register_details.email)

        return {"access_token": access_token, "token_type": "bearer", "message": "Successfully login!", "type": "success"}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def predict_health(data: HealthInput, db=Depends(get_connection), current_user: str = Depends(get_current_users)):
    try:
        ...
        # Safe insert (patientName optional)
        columns = "currentUser,healthScore,risk,diabetes,heartDisease,advice,predictionDate"
        values = [current_user, health_score, risk, round(diab_prob * 100, 1), round(heart_prob * 100, 1), advice, datetime.now()]
        if data.patientName:
            columns += ",patientName"
            values.append(data.patientName)

        sql_query = f"INSERT INTO HealthPredictData ({columns}) VALUES ({','.join(['%s']*len(values))})"
...
def user_feedback(feedback: feedbackData, db=Depends(get_connection), current_user: str = Depends(get_current_users)):
    try:
        cursor = db.cursor(buffered=True)
        cursor.execute(
            "INSERT INTO feedbackData (currentUser,accuracy,easyToUse,rating,suggestions,duration) VALUES (%s,%s,%s,%s,%s,%s)",
            (current_user, feedback.accuracy, feedback.easyToUse, feedback.rating, feedback.suggestions, datetime.now())
        )
        db.commit()
        return {"message": "Successfully submitted feedback", "reply": "Thank you so much for your precious time!"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def get_history(email: str, db=Depends(get_connection)):
    try:
        ...
        if not health_data_user:
            raise HTTPException(status_code=400, detail="History does not exist!")
