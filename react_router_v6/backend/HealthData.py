...
async def predict_health(data: HealthInput,db=Depends(get_connection),current_user:str=Depends(get_current_users)):
    try:
        ...
        # query = """
        #     INSERT INTO HealthPredictData 
        #     (currentUser, healthScore, risk, diabetes, heartDisease, advice, predictionDate) 
        #     VALUES (%s, %s, %s, %s, %s, %s, %s)
        # """

        # # 2. Package the data into a SINGLE tuple variable
        # # We use float() to ensure compatibility with MySQL
        # data_to_save = (
        #     str(current_user), 
        #     float(health_score), 
        #     str(risk), 
        ...
        # # 3. Execute passing exactly TWO arguments: the string and the tuple
